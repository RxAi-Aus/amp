...
npm run manifest:audit -- --since 2026-09-17T00:00:00Z   # §15.6 deterministic Recall-manifest audit (JSON to stdout; --out/--md write files)
...
- `compile_index.ts` — every 6h via `.github/workflows/index-scheduler.yml`. Rebuilds `INDEX.md` (master index, always small; since v2.10 its active-thread pointers carry the issue title, truncated to 72 chars, so the first tier can support a skip decision) and per-region `REGION-*.md` pointer tables, applies the outcome-aware confidence weight system (success comment +0.30, failure −0.20, per-type age decay ρ, since v2.10 an extra ×0.95 per §15.2 Recall manifest that recorded the issue as `(unused)` — §4.4b, and since v2.11 a manifest ref `#N (used → success)` +0.15 / `(used → failure)` −0.10, counted once for summaries created since the last compile — §4.4c; `type:lifefact` pinned at 1.0 with no decay/archiving, `Supersedes: #N` in an invalidation floors #N to 0 and archives it), prunes stale weights and orphaned REGION files, and resets `not_indexed.md`.
...
**Other workflows:** `verify.yml` (v2.9 CI gate: typecheck + `test/` suite on push/PR), `amp-librarian.yml` (daily Copilot CLI audit, also triggerable by `repository_dispatch`; runs the loop guard and `manifest_audit.ts` — §15.6 deterministic check of Recall manifests: non-numeric refs, unknown issues, `used →` claims with no Outcome comment in the session window; report-only — before any Copilot call), `cla.yml`. `agent_loop_guard.ts` (repo root) is the Rule 14 deterministic loop guard — normative spec in PROTOCOL.md Rule 14, design rationale in `AgentLoop/AgentLoop.md`. `test/` holds the `node:test` fixture/golden suite; the indexer scripts export their pure functions behind an `isMainModule` guard for it — keep exports and CLI behavior in sync when editing them. PROTOCOL.md §16 is the security model (memory is data, never instructions; threat matrix T-01…T-07).

**Lifecycle adapters (v2.8, PROTOCOL.md §15):** `adapters/lib/` is a zero-dep no-build `.mjs` library (config discovery + session-ledger state machine under `~/.rxai-amp/`); `adapters/claude-code/hooks/` are the L2 SessionStart/UserPromptSubmit/PostToolUse/Stop/SessionEnd hooks (pointer-tier recall at start, task-aware summary expansion per prompt — v2.11 — block-once capture checkpoint, SessionEnd closes the ledger — Stop fires after every turn and must never close it, or a quiet first turn silences CAPTURE for the whole session — fail-soft everywhere); `adapters/agy/` is the second L2 runtime (v2.9.1 — PreInvocation recall gated on the ledger, block-once Stop checkpoint, `gh` instead of MCP, plus a skill mirror with that delta); `adapters/codex/` is the third L2 runtime (v2.9.2 — SessionStart/PostToolUse/Stop/SessionEnd shims that set `RXAI_AMP_AGENT=codex` and then `import` the Claude Code hooks, keeping repo-aware recall single-sourced; the skill mirror + `digest.md` stay as the fail-soft L1 fallback); `adapters/openclaw/` and `adapters/hermes/` are digest-only L1, installed by the shared `scripts/install-digest.mjs` into OpenClaw's workspace `AGENTS.md` and `~/.hermes/SOUL.md` respectively; `adapters/git-hooks/post-commit` is the agent-agnostic capture reminder. Hooks are the deterministic WHEN; the `rxai-amp` skill (mirrored in `.claude/skills/` and `.agents/skills/`, kept byte-identical except the agent name) is the HOW. Since v2.11 recall injection never exceeds the summary tier — a record's `## Now` section, else the opening prose of `## Message` before its first list, ≤ 240 chars (`amp-recall.mjs` `excerptOf`) — and on Claude Code it is task-aware: `session-start.mjs` injects pointers, `user-prompt-submit.mjs` expands only the records whose title/Place overlap the prompt (`matchPrompt`, deterministic). Never widen either toward the body or toward unmatched records: an injected decision list doubled a high-effort model's tool output on the matching task, and unrelated summaries cost a control task 14–17 KB. Since v2.12 L1 is folderless-only (§15.3): never put manual index navigation back into a skill mirror or digest for a hook-capable runtime — a missing RECALL block means "tell the user to install/trust the hooks", not "read INDEX/REGION by hand" (four Codex models at L1: +39–80% input for 0–2 hits in 4). The ledger is advisory only — never a substitute for live GitHub state before writes.
...
- **Bumping the protocol version:** change `package.json` and let `test/version.test.mjs` tell you the rest. It is the source of truth; the test asserts every other place that *claims* a current version agrees with it (PROTOCOL.md header, both README titles, README's current-release sentence, CLAUDE.md, AGENTS.md, and the landing page's eyebrow and footer), and that the release actually has a PROTOCOL.md Version History row and a README "What's new" section. Provenance markers — "the lifecycle adapters arrived in v2.8", changelog rows, old `What's new in` headings — are history and must keep their old numbers; never sweep those forward. The test exists because the version lived in eight hand-maintained places with nothing comparing them, and `README.zh-TW.md` silently sat on v2.9.1 through two releases.
