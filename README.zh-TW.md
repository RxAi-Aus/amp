...
Issue 本文可以加一段 `## Now`（v2.11，可選）：一到三行散文，寫目標、目前狀態、
下一步，**不要列清單**。session 開始時 hook 注入的就是這一段（沒有的話，退而取
`## Message` 第一個清單之前的開頭文字，最多 240 字）；完整本文留在 issue 裡，
需要時再抓。`intent` 都應該寫一段。2026-09-23 量到的教訓：把 intent 的決策清單
整段注入，高 effort 的模型會把它當成待查清單，該題的工具輸出翻倍。

在 Claude Code 上（v2.11）session 開始只注入指標（每則一行標題），等 prompt 進來，
新的 `UserPromptSubmit` hook 才把標題／Place 和 prompt 字面重疊的紀錄展開成摘要
（去 stopword 的字根、中文二字組、明確的 `#N`），每則每個 session 只展開一次；
沒有相關記憶的 prompt 什麼都不注入——對照題上，不相關的摘要曾讓模型多讀 14–17 KB。
...
路徑也改成「讀 `INDEX.md` 的指標標題 → 直接抓對應 issue」，任何層級都不再為了找記憶
去讀 `REGION-*.md`（它留給瀏覽某個 Region 和發文前的重複檢查）。
...
- **`INDEX.md`** —— 主索引，永遠很小：每個區域一段摘要，加上活躍討論串的指標。自 v2.10 起，指標會帶上 Issue 標題（截斷至 72 字），因為第一層的職責就是讓代理程式判斷「哪些可以略過」——只有編號和權重是做不到這件事的。
...
```
每次編譯：      權重 = 舊權重 × 該類型衰減率 ρ × 未使用衰減 υ^u
成功留言：      權重 += 0.30      （留言含 **Outcome:** success）
失敗留言：      權重 −= 0.20      （留言含 **Outcome:** failure）
回憶清單 used： 權重 += 0.15 ／ −= 0.10（摘要 `## Recall` 記為 `#N (used → success|failure)`，v2.11）
中性／無標記：  不變
權重 < 0.10：   封存（從主表移到 Archived）
lifefact：      固定 1.0，永不衰減、永不封存
```

**`υ^u` 是什麼（v2.10，§4.4b）：** `ρ` 只回答「這則多舊了」。它跟著編譯排程走
——每天四次，不管那天有沒有事發生——所以「沒人用到的記憶」和「已經不相關的
記憶」衰減得一樣快。`υ` = 0.95，`u` 是 §15.2 回憶清單中把該 Issue 記為
`(unused)` 的次數：代理程式看到了它、判斷過了、然後略過。那是關於這則記憶
本身的證據，不是關於時鐘的。

`u` 每次編譯都從儲存重新推導（不累積在 `weights.json`），所以清單寫錯、之後
修正，下次編譯就會自行還原；只有明確標記 `(unused)` 的引用才計數；`u = 0` 與
v2.10 之前的算術完全等價，因此在清單出現之前，既有記憶庫不受任何影響。
Rule 12（lifefact 釘在 1.0）與 Rule 8（supersession 歸零）仍然優先。

**`(used → …)` 也算數（v2.11，§4.4c）：** v2.10 只讓回憶清單把權重往下拉，
沒有任何機制讓它往上推。由 hook 注入的記憶不會觸發 Stop 檢查點的義務（§15.4），
它該得的 `Outcome` 留言全靠代理程式自覺——而 §15 存在的理由正是自覺會失靈。
所以 `#N (used → success)` 現在 +0.15、`(used → failure)` −0.10：是直接留言的
一半（清單是 session 在自己日記裡的說法，不是討論串上的標記，證據力較弱），
而且只在摘要 issue 建立後的下一次編譯計一次。2026-09 的 A/B 裡被注入的三則
記憶，在 24 個有記憶的 session 中被正確引用 23 次，卻在十六天內全部因時間衰減
而封存——這條就是為它們補的。
