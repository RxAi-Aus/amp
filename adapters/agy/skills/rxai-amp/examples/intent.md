...
```
[FROM:agy→openclaw][REGION:projectx][PLACE:auth][TYPE:intent] Add OAuth flow for partner API
```
...
tool: issue_write
method: create
title: "[FROM:agy→openclaw][REGION:projectx][PLACE:auth][TYPE:intent] Add OAuth flow for partner API"
body:  "<the body block above>"
labels: ["from:agy", "type:intent", "unindexed"]
```
