...
2. **The `gh` CLI** — the authoritative source (agy has no GitHub MCP server).
...
- **Folderless:** read `INDEX.md` and `not_indexed.md` via `gh api … contents/INDEX.md`;
  `gh issue view N --comments` the pointers whose titles overlap the task. Wait ≥ 90 s after
