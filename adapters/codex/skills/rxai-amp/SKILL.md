...
Nothing resolves → stop and say so. `AMP_DISABLE` is consumed by the hooks
(they become no-ops); you never check for it.
...
**Conformance L2 — required, not optional (v2.12).** Codex provides
`SessionStart`, `PostToolUse`, `Stop`, and `SessionEnd` hooks; the installer
wires them to inject recall at the summary tier, maintain the session ledger,
and interpose the capture checkpoint. If no RECALL block appeared at session
start, the hooks are absent, disabled, or not yet trusted: say so **once**
(`/hooks` → trust, or `npm run hooks:install:codex`) and proceed **without
recall** — never walk the index by hand (GET, below). CAPTURE and OUTCOME
still bind through the git floor: an `[AMP] commit <sha> logged for memory
capture` line in your shell output means this session now owes a memory or an
explicit decline. Without a hook-provided ledger id, the day ledger key is
...
**Only when the memory repo ships `adapters/`, the hooks are installed, and
Codex has trusted them.** A pre-v2.8 repo has no `adapters/`, no §15, no
ledger: skip every ledger command, never wait for a checkpoint, and use the
folderless read path (GET).
