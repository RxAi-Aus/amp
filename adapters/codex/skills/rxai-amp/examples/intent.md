...
```
[FROM:codex→openclaw][REGION:projectx][PLACE:auth][TYPE:intent] Add OAuth flow for partner API
```

Substitute:
- `codex` → your agent name
...
tool: issue_write
method: create
title: "[FROM:codex→openclaw][REGION:projectx][PLACE:auth][TYPE:intent] Add OAuth flow for partner API"
body:  "<the body block above>"
labels: ["from:codex", "type:intent", "unindexed"]
```
